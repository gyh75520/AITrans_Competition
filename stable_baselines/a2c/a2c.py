...
class A2C(ActorCriticRLModel):
    ...
    def _train_step(self, step, obs, states, rewards, masks, actions, values, update, writer=None):
        """
        ...
        # print ('self.ent_coef_schedule.value(step)',self.ent_coef_schedule.value(step))
        ...
        td_map = {self.train_model.obs_ph: obs, self.actions_ph: actions, self.advs_ph: advs,
                  self.rewards_ph: rewards, self.learning_rate_ph: cur_lr,self.ent_coef_ph:self.ent_coef_schedule.value(step)}
    ...
    def learn(self, total_timesteps, callback=None, seed=None, log_interval=100, tb_log_name="A2C"):
        with SetVerbosity(self.verbose), TensorboardWriter(self.graph, self.tensorboard_log, tb_log_name) as writer:
            ...
            self.ent_coef_schedule = LinearSchedule(schedule_timesteps=int(1e6),initial_p=0.1,final_p=0.01)
            ...
            for update in range(1, total_timesteps // self.n_batch + 1):
                ...
                _, value_loss, policy_entropy = self._train_step(update * self.n_batch,obs, states, rewards, masks, actions, values, update,
                                                                 writer)
    ...
    def get_attention(self, observation, state=None, mask=None):
        if state is None:
        ...
        attention = self.step_model.get_attention(observation, state, mask)
